...
(define (chars->symbol ch input)
  (define bs (car (regexp-match #px"^[^\\s()\\[\\]{}\",'`;^_]*" input)))
  (define chs (bytes->string/utf-8 bs))
  (values (+ 1 (string-length chs)) (string->symbol (string-append (string ch) chs))))

(define (consume-as convert)
  (define (do-consume-as ch input name line column pos)
    (define-values (span val) (convert ch input))
    (datum->syntax #f val (vector name line column pos span)))
  do-consume-as)

(define (make-base-readtable [rt (current-readtable)])
  (make-readtable rt
                  #\\ 'non-terminating-macro (consume-as chars->symbol)
                  #\^ 'terminating-macro (consume-as char->symbol)
                  #\_ 'terminating-macro (consume-as char->symbol)))

;; Modified from scribble's at-reader implementation
;; - Use a base readtable that recognizes #\\ as symbols and #\^, #\_ as delimiters.
;; - Added conversion from repeated at-commands to nested applications
;;      @TFun{x}{e_1 e_2} => ((TFun "x") "e_1 e_2")
;; - Disable re-entering dispatch mode for #\@ inside commands
;;      @@ => the identifier @
;; - Attach source locations as the syntax property 'at-exp-srclocplus to the @-froms
;; - Keep the marker ('\n') after the last item in lines ({...}) rather than dropping it

(define (mintexpp-read [inp (current-input-port)])
  (parameterize ([current-readtable (make-default-at-readtable)])
    (read inp)))

(define (mintexpp-read-syntax [src default-src] [inp (current-input-port)])
  (parameterize ([current-readtable (make-default-at-readtable)])
    (read-syntax (src-name src inp) inp)))

(define (mintexpp-read-inside [inp (current-input-port)])
  (syntax->datum ((make-default-at-reader/inside) default-src inp)))

(define (mintexpp-read-syntax-inside [src default-src] [inp (current-input-port)] 
...
(define (dispatcher char inp source-name line-num col-num position
        ...
        [(and re:end (*match1 re:end))
         => (lambda (m)
              (if (and (zero? lvl) (not start-inside?))
                ;; OLD: drop a marker if it's after a last eol item
                ;;   (done-items (maybe-drop-marker r))
                ;; NEW: keep the last marker
                (done-items r)
  ...
  (cond
    ...
    [else
     (define rpfxs (get-rprefixes))

     (define (call-with-srclocp read-val [return (λ (arg0 . args) arg0)])
       (define-values (line-start column-start pos-start)
         (port-next-location inp))
       (define val (read-val))
       (define-values (line-end column-end pos-end)
         (port-next-location inp))
       (return val (srclocplus source-name
                               line-start
                               column-start
                               (and line-start (- line-end line-start))
                               column-end
                               pos-start
                               (and pos-start (- pos-end pos-start)))))

     (define (and-list . args)
       (and (andmap values args) args))

     (define-values (cmd cmd-srcp) (values #f #f))
     (define-values (datums datums-srcp) (values #f #f))
     (define-values (lines lines-srcp) (values #f #f))
     (cond
       ;; try get-lines first -- so @|{...}| is not used as a simple
       ;; expression escape, same for get-datums
       [(call-with-srclocp get-lines and-list)
        => (λ (the-lines/src)
             (set!-values (lines lines-srcp) (apply values the-lines/src)))]
       [(call-with-srclocp get-datums and-list)
        => (λ (the-datums/src)
             (set!-values (datums datums-srcp) (apply values the-datums/src))
             (set!-values (lines lines-srcp) (call-with-srclocp get-lines values)))]
       [(call-with-srclocp (λ () (get-escape-expr #t)) and-list)
        => (λ (expr/src)
             (set!-values (cmd cmd-srcp) (apply values expr/src)))]
       [else
        (set!-values (cmd cmd-srcp) (call-with-srclocp get-command values))
        (set!-values (datums datums-srcp) (call-with-srclocp get-datums values))
        (set!-values (lines lines-srcp) (call-with-srclocp get-lines values))])

     (define nested-stx
       (let loop ([cmd cmd] [cmd-srcp cmd-srcp]
                  [datums datums] [datums-srcp datums-srcp]
                  [lines lines] [lines-srcp lines-srcp])
         (define initial-stx
           (and (or datums lines)
                (append (or datums '()) (or lines '()))))

         (define allparts-stx
           (or (and cmd initial-stx (cons cmd initial-stx)) ; all parts
               initial-stx  ; no cmd part => just a parenthesized expression
               cmd  ; no datums/lines => simple expression (no parens)
               ;; impossible: either we saw []s or {}s, or we read a
               ;; racket expression
               (internal-error 'dispatcher)))

         (define ds (and datums (length datums)))
         (define ls (and lines  (length lines)))
         (define normalized-annotated-stx
           (syntax-property
            (syntax-property
             (if (syntax? allparts-stx)
                 allparts-stx
                 (datum->syntax #f allparts-stx
                                (vector source-name line-num col-num position
                                        (span-from position))
                                orig-stx))
             'scribble (list 'form ds ls))
            'at-exp-srclocplus
            `#((cmd . ,cmd-srcp) (datums . ,datums-srcp) (lines . ,lines-srcp))))

         (define processed-stx
           (syntax-post-processor normalized-annotated-stx))
     ...
     ;; wrap the prefixes around the result (e.g. quote, quasiquote, unquote, etc)
     (define prefixed-stx
       (let loop ([rpfxs rpfxs] [stx nested-stx])
         (if (null? rpfxs)
             stx
             (loop (cdr rpfxs) (list (car rpfxs) stx)))))

     (datum->syntax #f prefixed-stx
                    (vector source-name line-num col-num position
                            (span-from position))
                    orig-stx)]))
...
(define ((make-at-readtable-or-inside-reader inside-reader?)
  ...
  (define (make-cmd-rt command-readtable)
    ;; similar to plain Racket (scribble, actually), but with
    ;; `|' as a terminating macro characters (otherwise it behaves the
    ;; same; the only difference is that `a|b|c' is three symbols)
    ;;
    ;; Note that unlike Scribble's at-reader, command chars (@, \, etc) are not re-dispatched.
